...
def keyEvents(self, task):
    ...
    if self.keyMap["drop"] == 0:
        # self.changeHue(myFrame)
        # self.changeHue(yourFrame)
...
def changeHue( self, rec, amount ): #amount can be positive or negative
    color = rec.getFrameColor() #change if this syntax is wrong
    h, s, b = colorsys.rgb_to_hsv( color[0], color[1], color[2] )
    hue = restrain(h + amount)
    r, g, b = colorsys.hsv_to_rgb( hue, s, b )
    rec.FrameColor( Vec4( r, g, b, 1 ) ) 
